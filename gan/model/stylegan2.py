...
from gan.model.op import FusedLeakyReLU, conv2d_gradfix, fused_leaky_relu, upfirdn2d
...
class ToRGB(nn.Module):
    def __init__(
        self, in_channel, style_dim, upsample=True, blur_kernel=[1, 3, 3, 1], nb_var=3
    ):
        super().__init__()
    ...
    def forward(self, input, style, skip=None):
        ...
        return (
            (out, input_conved, skip, prev_rgb)
            if skip is not None
            else (out, input_conved)
        )
...
class Generator(nn.Module):
    ...
    def forward(
        ...
        for conv1, conv2, noise1, noise2, to_rgb in zip(
            ...
            skip, input_conved, prev_rgb_upsampled, prev_rgb = to_rgb(
                out, latent[:, i + 2], skip
            )
            # print("rgb ", skip.shape)
            if return_rgb:
                rgbs_saved["prev_rgb"][i // 2 + 2] = prev_rgb.detach().cpu().numpy()
                rgbs_saved["prev_rgb_upsampled"][i // 2 + 2] = (
                    prev_rgb_upsampled.detach().cpu().numpy()
                )
                rgbs_saved["input_conved"][i // 2 + 2] = (
                    input_conved.detach().cpu().numpy()
                )
        ...
        if self.var_rr and self.tanh_output:
            image[:, 0] = torch.tanh(
                image[:, 0]
            )  # assuming rr is the first variable ['rr', ...]
        if return_latents:
            if return_rgb:
