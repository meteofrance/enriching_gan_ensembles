...
def plot_2o_Estimators(estim_fake, estim_real, name, J, L, output_dir):
    ...
    for i, var in enumerate(["u", "v", "t2m"]):
        ...
        print(var)
        fig, axs = plt.subplots(1, J - 1, figsize=(21, 15), sharex=False, sharey=False)
        for j_up in range(1, J):
            ...
            axs[J - 1 - j_up].plot(
                [1 + j2 + (J - j_up) for j2 in range(j_up)],
            ...
            axs[J - 1 - j_up].plot(
                [1 + j2 + (J - j_up) for j2 in range(j_up)],
                estim_real[:, i, si:ei].max(axis=0),
                "k+",
                label="Max/Min AROME-EPS",
                linewidth=2.5,
            )
            axs[J - 1 - j_up].plot(
                [1 + j2 + (J - j_up) for j2 in range(j_up)],
                estim_real[:, i, si:ei].min(axis=0),
                "k+",
                linewidth=2.5,
            )

            axs[J - 1 - j_up].set_xticks(
                list(range(J - j_up + 1, J + 1)), fontsize="25"
            )
            axs[J - 1 - j_up].set_xlabel(
                r"Second-order scale $j_2 > j_1$", fontsize="25", fontweight="bold"
            )
            axs[J - 1 - j_up].title.set_text(
                r"First-order scale $j_1$={}, variable {}".format(J - j_up, var)
            )
        ...
        st = fig.suptitle(
            "Scattering {} estimators for {}, $J$={}, $L$={}".format(name, var, J, L),
            fontsize="28",
            fontweight="bold",
        )
        st.set_y(1.0)
        ...
        plt.savefig(
            output_dir + "scattering_{}_{}_{}_{}.png".format(var[1:-1], J, L, name),
            bbox_inches="tight",
        )
...
def plot_1o_Estimators(estim_fake, estim_real, name, J, L, output_dir):
    ...
    if len(estim_fake.shape) == 4:
        for i, var in enumerate(["$u$", "$v$", "$t_{2m}$"]):
            ...
            print(var)
            fig, axs = plt.subplots(1, L, figsize=(15, 6), sharex=False, sharey=False)
            for ell in range(L):

                axs[ell].plot(
                    [1 + j2 for j2 in range(J)],
                ...
                axs[ell].plot(
                    [1 + j2 for j2 in range(J)],
                    estim_real[:, i, :, ell].min(axis=0),
                    "r+",
                )

                axs[ell].set_xticks(list(range(1, J + 1)))
                axs[ell].set_xlabel(r"Scale $j_1$")
                axs[ell].title.set_text(
                    r"Orientation $ \ell=$" + str(ell) + r"$\frac{\pi}{L}$"
                )
            ...
            fig.subplots_adjust(bottom=0.05, top=0.9, left=0.05, right=0.95)
            st = fig.suptitle(
                "{} estimators for {}, $J$={}, $L$={}".format(name, var, J, L),
                fontsize="14",
            )
            st.set_y(1.0)

            plt.savefig(
                output_dir + "{}_{}_{}_{}.png".format(var[1:-1], J, L, name),
                bbox_inches="tight",
            )
        ...
        for i, var in enumerate(["$u$", "$v$", "$t_{2m}$"]):
            ...
            plt.plot(
                [1 + j2 for j2 in range(J)],
                estim_fake[:, i].mean(axis=0) + estim_fake[:, i].std(axis=0),
                "b--",
                label=r"Mean $\pm$ Std GAN",
            )
            plt.plot(
            ...
            plt.plot(
                [1 + j2 for j2 in range(J)],
                estim_fake[:, i].max(axis=0),
                "b+",
                label="Max/Min GAN",
            )

            plt.plot([1 + j2 for j2 in range(J)], estim_fake[:, i].min(axis=0), "b+")
            ...
            plt.plot(
                [1 + j2 for j2 in range(J)],
                estim_real[:, i].mean(axis=0) + estim_real[:, i].std(axis=0),
                "r--",
                label=r"Mean $\pm$ Std PEARO",
            )
            plt.plot(
            ...
            plt.plot(
                [1 + j2 for j2 in range(J)],
                estim_real[:, i].max(axis=0),
                "r+",
                label="Max/Min PEARO",
            )

            plt.plot([1 + j2 for j2 in range(J)], estim_real[:, i].min(axis=0), "r+")
            ...
            plt.savefig(
                "{}_{}_{}_{}.png".format(var[1:-1], J, L, name), bbox_inches="tight"
            )
