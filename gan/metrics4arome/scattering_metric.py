...
class scattering_metric:
    def __init__(
        self, J, L, shape, estimators, backend="numpy", frontend="numpy", cuda=False
    ):
        """
        Inputs:
    ...
    def scattering_rmse(self, real_data, fake_data):
        ...
        if self.count == 0:
            self.scat_real = scf.scatteringHandler(
                self.J,
                self.shape,
                L=self.L,
            ...
            self.scat_fake = scf.scatteringHandler(
                self.J,
                self.shape,
                L=self.L,
                frontend=self.frontend,
        ...
        for estName in self.estNames:
            ...
            rmse = np.sqrt(
                ((est_fake.mean(axis=0) - est_real.mean(axis=0)) ** 2).mean(axis=-1)
            )
    ...
    def scattering_distance_crude(self, real_data, fake_data):
        ...
        for estName in self.estNames:
            ...
            distances.append(
                wd.pointwise_W1(
                    getattr(self.scat_real, estName)(),
                    getattr(self.scat_fake, estName)(),
                ).mean(axis=-1)
            )
    ...
    def scattering_distance_refined(self, real_data, fake_data):
        ...
        for estName in self.estNames:
            ...
            distances.append(
                wd.pointwise_W1(
                    getattr(self.scat_real, estName)()[:, :, : self.J - 1],
                    getattr(self.scat_fake, estName)()[:, :, : self.J - 1],
                ).mean(axis=-1)
            )
    ...
    def scattering_sliced(
        self, real_data, fake_data, dir_repeats=4, dirs_per_repeat=128
    ):
        """
        ...
        for (
            estName
        ) in self.estNames:  # applying the same logic for different estimators
            ...
            rmse_std = np.sqrt(
                ((est_fake.std(axis=0) - est_real.std(axis=0)) ** 2).mean(axis=-1)
            )
            ...
            for i in range(est_real.shape[1]):
                distance_perVar[i] = sw.sliced_wasserstein(
                    est_real[:, i],
                    est_fake[:, i],
            ...
            distance = np.concatenate(
                (rmse_mean, rmse_std, np.array([swd]), distance_perVar), axis=0
            )
