...
def load_batch(path, number, CI, Shape=(3, 128, 128), option="fake"):
    ...
    if option == "fake":
        ...
        list_inds = random.sample(list_files, number)
        for i in range(number):
            Mat[i] = np.load(list_inds[i])[1:4, CI[0] : CI[1], CI[2] : CI[3]].astype(
                np.float32
            )

        Means = (
            np.load(path + "mean_with_orog.npy")[1:4]
            .reshape(1, 3, 1, 1)
            .astype(np.float32)
        )
        Maxs = (
            np.load(path + "max_with_orog.npy")[1:4]
            .reshape(1, 3, 1, 1)
            .astype(np.float32)
        )
...
for n in range(N_tests):
    ...
    plt.imshow(
        np.abs(ls_fake[0] - ls_real[0]) / ls_real[0], origin="lower", cmap="plasma"
    )
    plt.colorbar()
    plt.show()
    ...
    plt.imshow(
        np.abs(ls_fake[1] - ls_real[1]) / ls_real[1], origin="lower", cmap="plasma"
    )
    plt.colorbar()
    plt.show()
    ...
    plt.imshow(
        np.abs(ls_fake[2] - ls_real[2]) / ls_real[2], origin="lower", cmap="plasma"
    )
    plt.colorbar()
    plt.show()
