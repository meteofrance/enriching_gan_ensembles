...
for n in range(N_tests):
    ...
    splot.plot_increments(
        inc_real.mean(axis=0),
        inc_fake.mean(axis=0),
        1.3,
        ["u", "v", "t2m"],
        add_name="test",
    )
    ...
    splot.plot_structure(
        struct_real, p_list, 1.3, ["u", "v", "t2m"], add_name="test_PEARO"
    )
    splot.plot_structure(
        struct_fake, p_list, 1.3, ["u", "v", "t2m"], add_name="test_GAN"
    )

    splot.plot_structure_compare(
        struct_real[:, :, :3],
        struct_fake[:, :, :3],
        p_list[:3],
        1.3,
