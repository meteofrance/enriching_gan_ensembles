...
def plot2D_histo(var2var_f, var2var_r, levels, output_dir, add_name):
    ...
    fig, axs = plt.subplots(
        1, ncouples, figsize=(4 * ncouples, 2 * ncouples), sharex=True, sharey=True
    )
...
def var2Var_hist(data, bins, density=True):
    ...
    for tu in var_couples:
        ...
        if i != j:
            if type(bins) == int:
                bivariates[k], Bins[2 * k], Bins[2 * k + 1] = np.histogram2d(
                    data[:, i], data[:, j], bins=bins, density=density
                )
            elif type(bins) == np.ndarray:
                bivariates[k], _, _ = np.histogram2d(
                    data[:, i], data[:, j], bins=Bins[k], density=density
                )
            k += 1
...
def batchingvar2Var(data_list, bins, load_func, density=True):
    ...
    if density:
        ...
        for i in range(binsizes.shape[0] // 2):
            ...
            binareas[i] = (
                np.transpose(binsizes[2 * i : 2 * i + 1])
                * binsizes[2 * i + 1 : 2 * i + 2]
            )
...
def total_run(data_f, data_r, batching=False, levels=None):
    ...
    if not batching:
        ...
        bivariates_f, bins_f = batchingvar2Var(
            [("f", i) for i in range(100)], bins, simple_stupid_load
        )
        bivariates_r, bins_r = batchingvar2Var(
            [("r", i) for i in range(100)], bins, simple_stupid_load
        )
