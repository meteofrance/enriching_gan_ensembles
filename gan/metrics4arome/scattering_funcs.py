...
class scatteringHandler:
            ...
            if self.cuda:
                ...
                if Nbatch > 1024:  # if the data shape is too large for GPU memory,
                    ...
                    for i in range(steps):
                        ...
                        y = torch.tensor(
                            x[Slice * i : Slice * (i + 1)], dtype=torch.float32
                        ).cuda()
    ...
    def isotropicEstimator(self, order, scat=None):
        ...
        if order == 1:
            self.S1_j1 = np.reshape(
                scat, (scat.shape[0], scat.shape[1], self.J, self.L)
            )
            ...
            if self.order2_ind is None:
                ...
                self.order2_ind = [
                    [
                        [
                            [
                                self.L**2 * (j1 * (self.J - 1) - j1 * (j1 - 1) // 2)
                                + self.L * (j2 - j1 - 1)
            ...
            for j1 in range(self.J - 1):
                for j2 in range(self.J - j1 - 1):
                    S2_j1j2.append(
                        np.mean(scat[:, :, self.order2_ind[j1][j2]], axis=(-1, -2))
                    )
        ...
        for j1 in range(self.J - 1):
            for j2 in range(self.J - j1 - 1):
                for l1 in range(self.L):
                    ...
                    S2_j1j2l1.append(
                        np.mean(scat[:, :, self.order2_ind[j1][j2][l1]], axis=-1)
                    )
    ...
    def shapeEstimator(self, scat=None):
        ...
        for j1 in range(self.J - 1):
            for j2 in range(self.J - j1 - 1):
                ...
                s22_j1j2l1_ortho = []
                for l1 in range(self.L):
                    s22_j1j2l1_eq.append(scat[:, :, self.order2_ind[j1][j2][l1][l1]])

                    s22_j1j2l1_ortho.append(
                        scat[
                            :,
                            :,
                            self.order2_ind[j1][j2][l1][(l1 + self.L // 2) % self.L],
                        ]
                    )
                ...
                s22_j1j2.append(np.mean(s22_j1j2l1_eq / s22_j1j2l1_ortho, axis=0))
